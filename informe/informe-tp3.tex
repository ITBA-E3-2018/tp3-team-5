...
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (4,4);
\draw (0,4) -- node [pos=0.7,above right,anchor=south west] {$y_{2}y_{1}$} node [pos=0.7,below left,anchor=north east] {w} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\newenvironment{KarnaughEj2Moore}%
{
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (4,4);
\draw (0,4) -- node [pos=0.7,above right,anchor=south west]{$y_{2}y_{1}$} node [pos=0.7,below left,anchor=north east] {w$y_{3}$} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (4,2);
\draw (0,2) -- node [pos=0.7,above right,anchor=south west] {IS} node [pos=0.7,below left,anchor=north east] {y} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\newenvironment{KarnaughvuitEj3}%
{
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (4,2);
\draw (0,2) -- node [pos=0.7,above right,anchor=south west] {$y_{2}y_{1}$} node [pos=0.7,below left,anchor=north east] {w} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\newenvironment{KarnaughvuitEj2Mealy}%
{
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (4,2);
\draw (0,2) -- node [pos=0.7,above right,anchor=south west] {$y_{1}y_{2}$} node [pos=0.7,below left,anchor=north east] {w} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\begin{tikzpicture}[baseline=(current bounding box.north),scale=0.8]
\draw (0,0) grid (2,2);
\draw (0,2) -- node [pos=0.7,above right,anchor=south west] {y} node [pos=0.7,below left,anchor=north east] {w} ++(135:1);
%
\matrix (mapa) [matrix of nodes,
        column sep={0.8cm,between origins},
        row sep={0.8cm,between origins},
...
\begin{titlepage}
	...
	\vspace{1cm}
	{\scshape\Large 22.13 Electrónica III\par}
	\vspace{1.5cm}
	{\huge\bfseries Trabajo Practico Nro 3\par}
	\vspace{2cm}
	{\Large\itshape \begin{flushleft} \large
\emph{Grupo 5:}\\		%names
[.3cm]
...
conformados por compuertas lógicas. El primero de ellos convierte el
o los inputs del sistema y el estado actual de la FSM en el nuevo
estado de la FSM. Ese nuevo estado se guarda en Flip-Flops,
cada Flip-Flop se denomina variable de estado. De las cuales se necesitan
las suficientes como para representar en binario la cantidad de estados
...
combinacional, ademas del estado actual se tiene como input también
al input del circuito. De esta manera, según el problema, se puede
...
\hline 
Estado actual(y) & \multicolumn{4}{c|}{Proximo estado(Y)} & \multicolumn{4}{c|}{Output($B_{2}B_{1}$)}\tabularnewline
\hline 
\hline 
 & w=00 & 01 & 10 & 11 & w=00 & 01 & 10 & 11\tabularnewline
\hline 
0 & d & d & 1 & d & 11 & dd & 01 & 00\tabularnewline
\hline 
1 & d & d & 0 & d & 11 & dd & 10 & 00\tabularnewline
\hline 
\end{tabular}
\par\end{centering}
...
\begin{figure}[H]
	\begin{center}
    	\begin{KarnaughvuitEj1}
			\contingut{d,d,1,d,d,d,0,d}
      		\implicant{0}{2}{purple}
    	\end{KarnaughvuitEj1}	
	\end{center}
...
\begin{figure}[H]
		\begin{center}
    	\begin{KarnaughvuitEj1}
			\contingut{1,d,0,0,1,d,1,0}
      		\implicant{0}{5}{purple}
       		\implicantcostats{4}{6}{green}
    	\end{KarnaughvuitEj1}
    	\begin{KarnaughvuitEj1}
			\contingut{1,d,1,0,1,d,0,0}
      		\implicant{0}{5}{purple}
...
únicamente en función de estas compuertas.
...
\multicolumn{1}{c}{Próximo estado} &  & 
...
\begin{figure}[H]
	\begin{center}
	    \begin{KarnaughvuitEj2Mealy}
			\contingut{0,0,1,0,0,1,0,0}
      		\implicant{5}{5}{purple}
       		\implicant{2}{2}{purple}
    	\end{KarnaughvuitEj2Mealy}
    	\begin{KarnaughvuitEj2Mealy}
...
de la FSM implementada en Verilog utilizando únicamente compuertas
...
\caption{Circuito Compuertas NAND - Maquina de Moore}
\end{figure}
A continuación se realizó la simulación en Verilog, adjunta en la
carpeta, la cual dio el siguiente resultado en GTKwave:
...
\caption{Simulación en GTKwave}
\end{figure}
Para finalizar, se realizo en placa este circuito, el resultado
es el siguiente:

\begin{figure}[H]
\begin{center}
	\includegraphics[scale=0.1]{IMG_4557.jpg}
\end{center}
\caption{Realización física}
\end{figure}
...
\hline 
Estado actual &  & \multicolumn{1}{c}{Próximo estado} &  & \multicolumn{1}{c}{z(Output)} & \tabularnewline
\hline 
\hline 
...
practica se utilizó un diodo Zener de 3.3V en inversa. Los transistores
...
Como conclusión, se pudieron implementar exitosamente las tres maquinas de estado propuestas, tanto en su version de Moore como de Mealy.
 La metodología aplicada fue la misma para los tres casos.En primer lugar se planteó el grafo con los estados y las transiciones de cada uno, del mismo se identifican el numero de variables de estados. A continuación se escriben los estados y sus transiciones en forma de tabla de la cual se identifica el bloque de lógica combinacional  de entrada y el de salida. Finalmente se empleó el método de minimización por mapas de Karnaugh para cada uno de los bloques de lógica combinacional para llegar a la implementación menor costo.
